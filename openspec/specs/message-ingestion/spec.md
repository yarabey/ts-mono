...
## Purpose
Ingest free-form entries from a Telegram bot (text and transcribed voice) and an Alice (Yandex) webhook, storing unrecognized messages as pending `raw_entries` for later AI parsing, answering recognized query intents directly, and sending threshold-based notifications.
